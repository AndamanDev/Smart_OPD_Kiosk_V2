...
class VitalsProvider extends ChangeNotifier {
  ...
  Future<bool> saveToVitals({
    ...
    try {
      ...
      // payload — ค่าที่ไม่มีให้เป็น 0 (note เป็น "")
      // ยกเว้น height/weight: ส่งเฉพาะตอนมีค่าจริง เพื่อไม่ให้ 0 ไปทับค่าเดิม
      ...
      if (settings.workingMode == WorkingMode.scaleOnly) {
        ...
        if (bpPart != null) {
          final readingB = BloodPressureUtils.parseByDevice(
            bpPart,
            settings.bpDevice,
          );
