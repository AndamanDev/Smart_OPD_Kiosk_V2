...
class SettingsProvider extends ChangeNotifier {
  ...
  // -------- ค่าอ้างอิงความดัน (ไว้เปรียบเทียบกับผลที่วัดได้) --------
  int bpRefSys = 0; // ความดันบน (อ้างอิง)
  int bpRefDia = 0; // ความดันล่าง (อ้างอิง)
  ...
  // -------- Path --------
  String loginpath = '';
  String getpatientspath = '';
  String savevitalpath = '';
  ...
  Future<void> save({
    ...
    required String getpatientspath,
    required String savevitalpath,
    required String hospitalLogoPath,
  }) async {
    ...
    if (bpRefSys != null) this.bpRefSys = bpRefSys;
    if (bpRefDia != null) this.bpRefDia = bpRefDia;
    ...
    this.getpatientspath = getpatientspath;
    this.savevitalpath = savevitalpath;
    ...
    await prefs.setString('loginpath', loginpath);
    await prefs.setString('getpatientspath', getpatientspath);
    await prefs.setString('savevitalpath', savevitalpath);
    await prefs.setString('hospitalLogoPath', hospitalLogoPath);
    ...
    await prefs.setInt('bpRefSys', this.bpRefSys);
    await prefs.setInt('bpRefDia', this.bpRefDia);
  ...
  Future<void> loadFromStorage() async {
    ...
    bpRefSys = prefs.getInt('bpRefSys') ?? 0;
    bpRefDia = prefs.getInt('bpRefDia') ?? 0;
    ...
    loginpath = prefs.getString('loginpath') ?? '';
    getpatientspath = prefs.getString('getpatientspath') ?? '';
    savevitalpath = prefs.getString('savevitalpath') ?? '';
    hospitalLogoPath = prefs.getString('hospitalLogoPath') ?? '';
