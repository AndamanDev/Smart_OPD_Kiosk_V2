...
class _CardReaderCameraPageState extends State<CardReaderCameraPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Thai ID Scanner V2"),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            /// CARD DATA
            infoText("เลขบัตร: ${_getText("ID_Number")}"),
            infoText("ชื่อไทย: ${_getText("Thai_Name")}"),
            infoText(
              "ชื่อ: ${_getText("First_Name")} ${_getText("Last_Name")}",
            ),
            infoText("วันเกิด: ${_getText("Date_of_Birth")}"),
            infoText("วันออกบัตร: ${_getText("Date_of_Issue")}"),
            infoText("วันหมดอายุ: ${_getText("Date_of_Expiry")}"),
            infoText("ที่อยู่: ${_getText("Address")}"),
  ...
  String _getText(String tag) {
    try {
      return _boxResults.firstWhere((e) => e["tag"] == tag)["text"].toString();
    } catch (e) {
      return "-";
    }
  }
  ...
  String _buildLogText() {
    if (_boxResults.isEmpty) {
      return "Waiting for scan...";
    }

    String log = "";

    for (var r in _boxResults) {
      log += "${r["tag"]} : ${r["text"]}\n";
    }
